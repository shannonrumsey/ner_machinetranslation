...
def translate_entity(entity_name, language_code):
    ...
    if entity_name == "Best Actor": # just some small mistakes that annoy me
        if language_code == "fr":
            return "Meilleur Acteur"
        elif language_code == "es":
            return "mejor Actor"
        elif language_code == "it":
            return "Miglior Attore"
    if entity_name == "US":
        entity_name = "United States"
    if entity_name == "##ja Cat":
        entity_name = "Doja Cat"
    ...
    try:
        # search for similar pages (I stopped using the exact page because often it wasnt found or was wrong)
        ...
        if search_results:
            ...
            if page_en.exists():
                if language_code in page_en.langlinks:
                    ...
                    # remove parentheses and their content using regex. (this is to avoid getting outputs like Titanic (pelicula)
        ...
        print(f"Error occurred while processing '{entity_name}': {e}")
        return entity_name
            ...
            if len(ner_results_en)  != len(ner_results_lang):
                ...
                for result_index in range(len(ner_results_en)):
                    if ner_results_en[result_index]["entity_group"] != ner_results_lang[result_index]["entity_group"]:
                        ...
                        lang_ent = ner_results_lang[result_index]["word"]
