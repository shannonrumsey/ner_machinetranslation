...
def remove_punctuation(text):
    # Add a space before the question mark and then remove all other punctuation
    text = re.sub(r'\?', ' ?', text)  # add a space before question mark
...
def add_underscores(lookup_table):
    lookup_table = {key: "_".join(value.split()) for key, value in lookup_table.items()}
    return lookup_table
...
df_es.to_csv('/Users/darianlee/PycharmProjects/entity_preprocessing/df_es_underscore.csv', index=False, encoding='utf-8')
df_it.to_csv('/Users/darianlee/PycharmProjects/entity_preprocessing/df_it_underscore.csv', index=False, encoding='utf-8')
